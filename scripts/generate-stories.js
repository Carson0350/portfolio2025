...
const STORY_POINTS = {
  ...
  // Epic 2
  '2.1': 2, '2.2': 2, '2.3': 2, '2.4': 3, '2.5': 2, '2.6': 2, '2.7': 3,
  // Epic 3
  '3.1': 1, '3.2': 2, '3.3': 2, '3.4': 2, '3.5': 3, '3.6': 2, '3.7': 2,
  // Epic 4
  '4.1': 2, '4.2': 2, '4.3': 1, '4.4': 2, '4.5': 2, '4.6': 2,
  // Epic 5
  '5.1': 2, '5.2': 2, '5.3': 1, '5.4': 2, '5.5': 2, '5.6': 2,
  // Epic 6
  '6.1': 2, '6.2': 2, '6.3': 3, '6.4': 2, '6.5': 2, '6.6': 2, '6.7': 2,
  // Epic 7
  '7.1': 2, '7.2': 1, '7.3': 2, '7.4': 2, '7.5': 3, '7.6': 2, '7.7': 2,
  // Epic 8
  '8.1': 2, '8.2': 1, '8.3': 2, '8.4': 2, '8.5': 3, '8.6': 2, '8.7': 2,
  // Epic 9
  '9.1': 3, '9.2': 2, '9.3': 2, '9.4': 2, '9.5': 1, '9.6': 2, '9.7': 2,
};

// Priority mapping
const PRIORITIES = {
  1: 'P0', 2: 'P0', 3: 'P1', 4: 'P1', 5: 'P1', 6: 'P0', 7: 'P1', 8: 'P2', 9: 'P0'
};
...
function parseEpicFile(epicNumber) {
  const epicFile = path.join(PRD_DIR, `epic-${epicNumber}-*.md`);
  const files = fs.readdirSync(PRD_DIR).filter(f => f.startsWith(`epic-${epicNumber}-`));
  
  if (files.length === 0) {
    console.log(`⚠️  Epic ${epicNumber} file not found`);
    return [];
  }
  ...
  while ((match = storyRegex.exec(content)) !== null) {
    ...
    // Extract user story
    const userStoryMatch = body.match(/As a (.+?),\nI want to (.+?),\nso that (.+?)\./s);
...
// Main execution
console.log('🚀 Generating story files from PRD epics...\n');
...
for (let epicNumber = 1; epicNumber <= 9; epicNumber++) {
  ...
  console.log(`   Found ${stories.length} stories in Epic ${epicNumber}`);
}

console.log(`\n✨ Complete! Generated ${totalStories} story files in ${STORIES_DIR}`);
console.log(`\n📁 Next steps:`);
console.log(`   1. Review generated files in docs/stories/`);
console.log(`   2. Add technical notes to stories as needed`);
console.log(`   3. Start development with Story 1.1`);
